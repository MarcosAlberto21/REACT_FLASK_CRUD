...
class InputSchema(Schema):
    salon = fields.String(required=True)
...
@m.route('/add', methods=['POST'])
def post_data():
	try:
			input_data = request.json
			input_schema.load(input_data)
			salon = input_data['salon']
			horario = input_data['horario']
			decripcion = input_data['descripcion']
			mat.insertar(salon, horario, decripcion)
			response = {'data': 'Node created successfully', 'salon': salon, 'horario': horario, 'descripcion': decripcion}
...
@m.route('/grafica', methods=['GET'])
def grafica():
	try:
		mat.graficar()
		with open("matriz.dot", "r") as archivo_dot:
